...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['idbox' . $x]){
			$delete = "delete from `leave` where id = '" . $_POST['idbox' . $x] . "'";
...
<h3 class="wintitle">Leave Type Entry</h3>
<form method="post">
<table width=100% border="0" cellpadding="4" cellspacing="0">
	<tr>
		<td width=30%>Name</td>
		<td width=5%></td>
		<td width=60% colspan=2>Description</td>
	</tr>
	<tr>
	...
	<?php
	$x = 0;
	$select = "select * from `leave` order by id desc";
	...
	<tr>
		...
		<td><input type="text" name="day<?php echo $x; ?>" style="width:100%" value="<?php echo $row['day']; ?>"></td>
