...
if (isset($_POST['save'])){
	$insert = "insert into `ethnic_race` (
		`id`,
		`name` ,
		`description` 
		)
...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['idbox' . $x]){
			$delete = "delete from ethnic_race where id = '" . $_POST['idbox' . $x] . "'";
...
<h3 class="wintitle">Ethnic Race</h3>
<body id="innerframe">
<form method="post">
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<?php
	$x = 0;
	$select = "select * from ethnic_race order by id desc";
