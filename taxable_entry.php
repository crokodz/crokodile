...
if (isset($_POST['save'])){
	$insert = "insert into `taxable_entry` (
		`id`,
		`name` ,
		`description`,
		acct_name,
...
if (isset($_POST['update'])){
	for ($x=0; $x < $_POST['count']; $x++){
		$update = "update taxable_entry set
			name = '" . $_POST['name' . $x] . "',
...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['idbox' . $x]){
			$delete = "delete from taxable_entry where id = '" . $_POST['idbox' . $x] . "'";
...
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<?php
	$x = 0;
	$select = "select * from taxable_entry where name!='LHRD' order by id desc";
