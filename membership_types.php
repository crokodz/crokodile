...
if (isset($_POST['save'])){
	$insert = "insert into `membership_types` (
		`id`,
		`name` ,
		`description` 
		)
...
if (isset($_POST['update'])){
	for ($x=0; $x < $_POST['count']; $x++){
		$update = "update membership_types set 
			name = '" . $_POST['name' . $x] . "',
...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['idbox' . $x]){
			$delete = "delete from membership_types where id = '" . $_POST['idbox' . $x] . "'";
...
<h3 class="wintitle">Membership Type</h3>
<body id="innerframe">
<form method="post">
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<?php
	$x = 0;
	$select = "select * from membership_types order by id desc";
