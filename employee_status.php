...
if (isset($_POST['save'])){
	$insert = "insert into `employee_status` (
		`id`,
		`name` ,
		`description` 
		)
...
if (isset($_POST['update'])){
	for ($x=0; $x < $_POST['count']; $x++){
		$update = "update employee_status set 
			name = '" . $_POST['name' . $x] . "',
...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['idbox' . $x]){
			$delete = "delete from employee_status where id = '" . $_POST['idbox' . $x] . "'";
...
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<?php
	$x = 0;
	$select = "select * from employee_status order by id desc";
