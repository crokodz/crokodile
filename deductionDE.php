...
function getOnhold($em_id){
	$select = "select sum(`amount`) as `onhold` from employee_deduction where 
		em_id = '" . $em_id . "' and 
		status = 'onhold' ";
	$result =  mysql_query($select, connect());
...
if(isset($_POST['onhold'])){
	$update = "update employee_deduction set status = 'onhold', deleted = '" . $_SESSION['user'] . "', deleted_date = curdate() where  em_id = '" . $_GET['id'] . "' and `status` = 'pending'  ";
	mysql_query($update, connect());
	}
	
if(isset($_POST['pending'])){
	$update = "update employee_deduction set status = 'pending', deleted = '', deleted_date = '' where  em_id = '" . $_GET['id'] . "' and `status` = 'onhold'  ";
	mysql_query($update, connect());
	}
...
?>
<tr>
	<td align="right" colspan=9><input value="Put all ON-HOLD Status to Pending Status" type="submit" name="pending"></td>
</tr>
<?php
$diabled = "disabled";
$name = "On Hold";
...
?>
<tr>
	<td align="right" colspan=9><input value="Put all Pending Status to ON-HOLD Status" type="submit" name="onhold"></td>
</tr>
<?php
$name = "del";
}
...
<table width=100% border="0" cellpadding="4" cellspacing="0" rules="all">
<?php 
$select = "select *, max(ded_amount) as ded_amt from employee_deduction where em_id = '" . $_GET['id'] . "' and status != 'deleted' group by sub_id order by deduct_id asc ";
$result = mysql_query($select, connect());
...
?>
<tr>
	<td align="left"><?php echo $row['name'] . $row['sub_id']; ?></td>
	...
	<td width=10% align="right"><?php echo roundoff($row['ded_amt'],2); ?></td>
	<td width=10% align="center"><?php echo $row['date_granted']; ?></td>
	<td width=10% align="center"><?php echo $row['date_effectivity']; ?></td>
	<td width=10% align="right"><?php echo roundoff($posted['posted']); ?></td>
	<td width=10% align="right"><?php echo roundoff($pending['pending']); ?></td>
	<td width=7%><input type="submit" name="delete" value="<?php echo $name; ?>" onClick="deleteID(this.form.id,'<?php echo $row['sub_id']; ?>');" <?php echo $diabled; ?>></td>
</tr>
<?php
}
?>
