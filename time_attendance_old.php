...
function getndl($company,$ti,$to){
	$select = "select night_differential, night_differential_below from company where id = '" . $company . "'";
	$result = mysql_query($select, connect());
	$row = mysql_fetch_array($result,MYSQL_ASSOC);
	
	$nd = h2m($row['night_differential']);
	$ndb = h2m($row['night_differential_below']);
	if($ti < $ndb){
		$b = $ndb - $ti;
		}
	if($to > $nd){
...
if(isset($_POST['save'])){
	for ($x=0;$x<$_POST['count'];$x++){
		...
		$start_ot = $_POST['start_ot' . $x];
		$end_ot = $_POST['end_ot' . $x];
		$cb = $_POST['cb' . $x];
		$cbot = $_POST['cbot' . $x];
		$otrem = $_POST['otrem' . $x];
		...
		if ($_POST['allowed_ot' . $x] == "NO" or $_POST['status' . $x] == 'UNFILED' or $ot < 0 or $_POST['status' . $x] == 'ABSENT' or $_POST['status' . $x] == 'AWOL'){
			$ot = "0";
			}
		if ($_POST['allowed_late' . $x] == "NO" or $_POST['status' . $x] == 'RESTDAY' or $_POST['status' . $x] == 'UNFILED' or $late < 0 or $_POST['status' . $x] == 'ABSENT' or $_POST['status' . $x] == 'AWOL'){
			$late = "0";
			}
		if ($_POST['allowed_ut' . $x] == "NO" or $_POST['status' . $x] == 'RESTDAY' or $_POST['status' . $x] == 'UNFILED' or $ut < 0 or $_POST['status' . $x] == 'ABSENT' or $_POST['status' . $x] == 'AWOL'){
			$ut = "0";
			}
		...
		if(($_POST['status' . $x] == 'LEGAL HOLIDAY' and $timein == '00:00') or ($_POST['status' . $x] == 'SPECIAL HOLIDAY' and $timein == '00:00')){
			$ut = "0";
			$ot = "0";
			$late = "0";
			}
			
		if(($_POST['status' . $x] == 'LEGAL HOLIDAY' and $cbot == "checked") or ($_POST['status' . $x] == 'SPECIAL HOLIDAY' and $cbot == "checked")){
			$ut = "0";
			$late = "0";
			$ot = $total;
			}
...
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<tr>
		...
		<td align="left"><select name="fyy"><option><?php echo $fyy; ?></option><?php echo $yy; ?></select><select name="fmm"><option><?php echo $fmm; ?><?php echo $mm; ?></select><select name="fdd"><option><?php echo $fdd; ?><?php echo $dd; ?></select></td>
	</tr>
	<tr>
		<td align="left"><?php echo getword("to"); ?></td>
		<td align="left"><select name="tyy"><option><?php echo $tyy; ?><?php echo $yy; ?></select><select name="tmm"><option><?php echo $tmm; ?><?php echo $mm; ?></select><select name="tdd"><option><?php echo $tdd; ?><?php echo $dd; ?></select></td>
	</tr>
	<tr>
...
<table width=100% border="0" cellpadding="4" cellspacing="0">
	...
	<tr>
		<td colspan=2 align="left">
			...
			if($_SESSION['user'] == 'mso'){
				?>
				<option value="ALL MSO">ALL MSO</option>
				<?php
				}
			while ($data = mysql_fetch_array($result_data,MYSQL_ASSOC)){
			?>
	...
	<tr>
		<td colspan=2 align="left"><input type="button" name="search" value="Open By Company & PayCode" onclick="openwindow('time_attendance_multi.php?from='+this.form.fyy.value+'-'+this.form.fmm.value+'-'+this.form.fdd.value+'&to='+this.form.tyy.value+'-'+this.form.tmm.value+'-'+this.form.tdd.value+'&id='+this.form.company.value,400,400)"></td>
	</tr>
</table>
<?php
}
...
<tr>
	...
	<td width="10px" align="center"><?php echo getword("AUTH OT"); ?></td>
</tr>
<?php
...
while ($row = mysql_fetch_array($result,MYSQL_ASSOC)){
	...
	if ($row['posted_id']){
		?>
		<tr <?php if ($row['status'] == 'UNFILED' or $dayoftheweek == 'Sat' or $dayoftheweek == 'Sun') { echo 'bgcolor="#4CC552"'; $bbg=""; } else{ $bbg='bgcolor="lightyellow"'; }?> <?php echo $bg; ?>>	
			<td><?php echo $dayoftheweek; ?></td>
			<td><?php echo $row['status']; ?></td>
			<td><?php echo $row['trxn_date']; ?></td>
			<td><?php echo $row['shift_code']; ?></td>
			<td <?php echo $bbg; ?>><?php echo $row['trxn_time_in']; ?></td>
			<td <?php echo $bbg; ?>><?php echo $row['trxn_time_out']; ?></td>
			<td><?php echo m2h($row['total']); ?></td>
			<td><?php echo m2h($row['late']); ?></td>
			<td><?php echo m2h($row['ut']); ?></td>
			<td><?php echo m2h($row['otin']); ?></td>
			<td><?php echo m2h($row['otout']); ?></td>
			...
			<td><input type="checkbox" name="cbot<?php echo $x; ?>" id="cbot<?php echo $x; ?>" <?php echo $row['cbot']; ?> disabled></td>
			
			<input type="hidden" name="cb<?php echo $x; ?>" id="cb<?php echo $x; ?>" value="1">
			<input type="hidden" name="start_ot<?php echo $x; ?>" value="<?php echo cutz($row['start_ot']); ?>">
			<input type="hidden" name="end_ot<?php echo $x; ?>" value="<?php echo cutz($row['end_ot']); ?>">
	...
	?>
		<tr <?php if ($row['status'] == 'UNFILED' or $dayoftheweek == 'Sat' or $dayoftheweek == 'Sun') { echo 'bgcolor="#4CC552"'; }?> <?php echo $bg; ?>>
			...
			<td>
			<select name="status<?php echo $x; ?>" style="width:100%;">
				<?php
			...
			<td><input type="text" name="in<?php echo $x; ?>" value="<?php echo cutz($row['trxn_time_in']); ?>" maxlength="5" style="width:40px;background-color:yellow;border:1px solid #000;"></td>
			<td><input type="text" name="out<?php echo $x; ?>" value="<?php echo cutz($row['trxn_time_out']); ?>" maxlength="5" style="width:40px;background-color:yellow;border:1px solid #000;"></td>
			<td><?php echo m2h($row['total']); ?></td>
			<td><?php echo m2h($row['late']); ?></td>
			<td><?php echo m2h($row['ut']); ?></td>
			<td><input type="text" name="otin<?php echo $x; ?>" value="<?php echo cutz($row['ot_time_in']); ?>" maxlength="5" style="width:40px;background-color:yellow;border:1px solid #000;"></td>
			<td><input type="text" name="otout<?php echo $x; ?>" value="<?php echo cutz($row['ot_time_out']); ?>" maxlength="5" style="width:40px;background-color:yellow;border:1px solid #000;"></td>
			<td><input type="text" name="ot<?php echo $x; ?>" value="<?php echo m2h($row['ot']); ?>" style="width:40px;" maxlength="5" readonly></td>
			<td><input type="text" name="otx<?php echo $x; ?>" value="<?php echo m2h($row['otx']); ?>" style="width:40px;" maxlength="5" readonly></td>
			<td><input type="text" name="nd<?php echo $x; ?>" value="<?php echo m2h($row['nd']); ?>" style="width:40px;" maxlength="5" readonly></td>
			<td><input type="text" name="otrem<?php echo $x; ?>" value="<?php echo $row['otremarks']; ?>" style="width:120px;"></td>
			<td><input type="checkbox" name="cbot<?php echo $x; ?>" id="cbot<?php echo $x; ?>" <?php echo $row['cbot']; ?>></td>
...
?>
<tr>
	<td colspan=13 align="right"><input type="submit" name="save" value="update"></td>
</tr>
<?php
}
?>
