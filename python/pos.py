...
from reportlab.lib.pagesizes import letter, portrait,landscape, A4,legal,POX
	...
	def __init__(self):
		self.CreateReport()
                ...
                SimpleDocTemplate(file, showBoundary=0, leftMargin=.5*inch, rightMargin=.5*inch,
                                                bottomMargin=1*inch, topMargin=.8*inch,pagesize = portrait(POX)).build(self.__lst,onFirstPage=self.myFirstPage,
                                                onLaterPages=self.myLaterPage)

	def myFirstPage(self, canvas, doc):
		canvas.saveState()
		pdfmetrics.registerFont(TTFont('arial', 'FUTURAL.TTF'))
