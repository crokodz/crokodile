...
function getOnhold($em_id){
	$select = "select sum(`amount`) as `onhold` from employee_non_taxable where 
		em_id = '" . $em_id . "' and 
...
if(isset($_POST['delete'])){
	...
	$update = "update employee_non_taxable set status = 'deleted', deleted = '" . $_SESSION['user'] . "', 		deleted_date = curdate() where id = '" . $_POST['id'] . "'";
	mysql_query($update, connect());
	}
	
if(isset($_POST['onhold'])){
	$update = "update employee_non_taxable set status = 'onhold', deleted = '" . $_SESSION['user'] . "', deleted_date = curdate() where  em_id = '" . $_GET['id'] . "' and `status` = 'pending'  ";
	mysql_query($update, connect());
	
	$update = "update employee_non_taxable set status = 'onholddeminimis', deleted = '" . $_SESSION['user'] . "', deleted_date = curdate() where  em_id = '" . $_GET['id'] . "' and `status` = 'Deminimis'  ";
	mysql_query($update, connect());
	}
	
if(isset($_POST['pending'])){
	$update = "update employee_non_taxable set status = 'pending', deleted = '', deleted_date = '' where  em_id = '" . $_GET['id'] . "' and `status` = 'onhold'  ";
	mysql_query($update, connect());
	
	$update = "update employee_non_taxable set status = 'Deminimis', deleted = '', deleted_date = '' where  em_id = '" . $_GET['id'] . "' and `status` = 'onholddeminimis'  ";
	mysql_query($update, connect());
	}
...
<h3 class="wintitle">Employee Non Taxable Income</h3>
<form method="post">
<?php
if($row['em_id']){
?>
...
<tr>
	<td width=20%>
		...
		function getSelectedId(text, li) {
			...
			self.location='index.php?menu=eontaxable&idd='+myData[0];
			}
		</script>
	</td>
</tr>
...
<tr>
	<td width=80%>
		...
		<?php
		$select = "select * from nontaxable_entry order by name asc";
		...
		|
		Deminimis <input type="checkbox" name="deminimis">
	</td>
...
if (isset($_POST['search'])){
	...
	echo '<script>';
	if (empty( $_GET['page'])){
		echo 'self.location = "index.php?menu=eontaxable&page=1&keyword=' . $keyword . '&company=' . $company . '";';
		}
	else{
		echo 'self.location = "index.php?menu=eontaxable&page=' . $_GET['page'] . '&keyword=' . $keyword . '&company=' . $company . '";';
		}
	echo '</script>';
	}
...
$pagelist = $p->pageList($_GET['page'], $pages,'index.php?menu=eontaxable&keyword=' . $_GET['keyword'] . '&company=' . $_GET['company']);
##
...
?>
	...
	<tr style="cursor:pointer;" id="emlist">
		<td align="left" onclick="self.location='index.php?menu=eontaxable&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['em_id']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=eontaxable&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['name']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=eontaxable&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['department']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=eontaxable&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['em_number']; ?></td>
	</tr>
	<?php
	$x++;
	}
