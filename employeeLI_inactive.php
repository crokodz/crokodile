...
if (isset($_POST['delete'])){
	for ($x=0; $x < $_POST['count']; $x++){
		if ($_POST['id' . $x]){
			$update = "update employee set status='active', username='' where em_id = '" . $_POST['id' . $x] . "'";
...
if (isset($_POST['search'])){
	...
	echo '<script>';
	if (empty( $_GET['page'])){
		echo 'self.location = "index.php?menu=em_inactive&page=1&keyword=' . $keyword . '&company=' . $company . '";';
		}
	else{
		echo 'self.location = "index.php?menu=em_inactive&page=' . $_GET['page'] . '&keyword=' . $keyword . '&company=' . $company . '";';
		}
	echo '</script>';
	}
$select = "select count(*) as total from employee where status='deleted' and name like '%" .  $_GET['keyword'] . "%'  and pay_id like '" . $_SESSION['dep'] . "' and company_id like '%" . $_GET['company'] . "';";
...
$pagelist = $p->pageList($_GET['page'], $pages,'index.php?menu=em_inactive&keyword=' . $_GET['keyword'] . '&company=' . $_GET['company']);
##

if ($_SESSION['company'] == '0'){
	$select = "select * from employee where status='deleted' and name like '%" .  $_GET['keyword'] . "%'  and pay_id like '" . $_SESSION['dep'] . "' and company_id like '%" . $_GET['company'] . "'  order by em_id asc  LIMIT " . $start . " , " . $limit;
	$result = mysql_query($select, connect());
	}
else{
	if (empty($_GET['company'])){
		echo "<script>";
		//echo 'self.location = "index.php?menu=1&page=em_inactive&keyword=&company=' . $_SESSION['company'] . '";';
		echo "</script>";
		}
	else{
...
<h3 class="wintitle"><?php echo getword("Inactive Employee List"); ?></h3>
<form method="POST">
<table width=100% border="0" cellpadding="4" cellspacing="0">
	<tr>
		...
		new Ajax.Autocompleter("keyword","hint","server_inactive.php", {afterUpdateElement : getSelectedId});
		...
		<td width=20% align="right"><a href="javascript:void(0);" onclick="ClickButton('delete')">undo</a><input type="submit" name="delete" id="delete" value="delete" style="display:none;"></td>
	</tr>
	<tr>
