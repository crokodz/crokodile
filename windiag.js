...
function f_dialogOpen (s_url, s_title, s_features,eve) {
	if (!window.a_windows)
		window.a_windows = [];
	...
	fullwidth = a_docSize[0];
	fullheight = a_docSize[1];
	
	var n_width  = a_features.width  ? parseInt(a_features.width)  : fullwidth-10;
	var n_height = a_features.height ? parseInt(a_features.height) : fullheight-35;
	
	e_window.style.left = (a_features.left ? parseInt(a_features.left) : (((a_docSize[0] - 4) - n_width)  / 2) + a_docSize[2]) + 'px';
	e_window.style.top  = (a_features.top  ? parseInt(a_features.top)  : (((a_docSize[1] - 30) - n_height) / 2) + a_docSize[3]) + 'px';
	...
	e_window.innerHTML = 
		...
		'</span><img src="close_icon.gif" onclick="top.f_dialogClose();" onmousedown="return false;" style="float:right"></th></tr><tr><td><iframe width="' + n_width +
		'" height="' + n_height +
		'" src="' + s_url + '" name="myframetop"></iframe></td></tr></table>';
	}
	else{
	e_window.innerHTML = 
		'<table border="2" class="' +
		...
		'"><tr><th onmousedown="f_dragStart(' + n_nesting + ', event)" onmouseup="f_dragEnd()" onmousemove="f_dragProgress(event)" onselectstart="return false"><span style="float:left"">' +
		(s_title ? s_title : 'Dialog Window') +
		'</span><img src="close_icon.gif" onclick="window.parent.location=window.parent.location;top.f_dialogClose();" onmousedown="return false;" style="float:right"></th></tr><tr><td><iframe width="' + n_width +
		'" height="' + n_height +
		'" src="' + s_url + '" name="myframetop"></iframe></td></tr></table>';
	}
...
function f_dragProgress (e_event) {
	...
	lEfT = parseInt(window.e_draggedWindow.style.left) - window.n_mouseX + n_newMouseX;
	if(lEfT < 4){
		window.e_draggedWindow = null;
		}
	tOp = parseInt(window.e_draggedWindow.style.top)  - window.n_mouseY + n_newMouseY
	if(tOp < 4){
		window.e_draggedWindow = null;
		}
		
	window.e_draggedWindow.style.left = lEfT + 'px';
	window.e_draggedWindow.style.top  = tOp + 'px';
