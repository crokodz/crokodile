<h3 class="wintitle">Absences Summary</h3>
...
<table width=100% border="0" cellpadding="4" cellspacing="0" rules="all">
	...
	if ($id){
		foreach ($id as $paycode){
			$select = "select fname,lname,mname, sum(late) as lt, count(trxnname) as days, division,em_id from transaction left join employee using (`em_id`) where trxn_date between '" . $_POST['fdate'] . "' and '" . $_POST['tdate'] . "' and (`transaction`.status = 'ABSENT' or `transaction`.status = 'UNFILED' or `transaction`.status = 'LWOP' or `transaction`.status = 'SUS' or `transaction`.status = 'AWOL' or `transaction`.status = 'NO WORK') and employee.status = 'active' and transaction.pay_id = '" . $paycode . "' group by em_id order by count(trxnname) desc" ;
			...
			?>
			<tr  style="cursor:pointer;" id="emlist" onclick="self.location='index.php?menu=51det&fdate=<?php echo $_POST['fdate']; ?>&tdate=<?php echo $_POST['tdate']; ?>&em_id=<?php echo $row['em_id']; ?>'">
				<td><?php echo $name; ?></td>
