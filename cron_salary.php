...
<div class="salary">New Salary</div><div class="searchinline">&nbsp;<input type="text" name="newsalary" id="newsalary"></div><br>
...
<div class="salary"><input type="submit" name="savesal" id="savesal" value="Save"></div>
</div>
<br>
<br>
...
<div style="display:inline-block;">
	<input type="submit" name="upssal" id="upssal" value="upload xml" onclick="return onups('file_name');">
	<a href="javascript:void.void(0)" onclick="showTip(this)">HELP</a>
	&nbsp;&nbsp;&nbsp;
	<input type="submit" name="clearsal" value="Clear">
</div>
<div class="LiHeader">
<div>
...
<?php
$select = "select * from cronjob where `username` = '" . $_SESSION['user'] . "' and type = 'sal' and `status` = 0";
...
<div style="<?php echo $sytle; ?>">
	...
	<div id="h5"><input type="checkbox" name="cb<?php echo $x; ?>" value="<?php echo $row['id']; ?>" <?php echo $disabled; ?> <?php echo $checked; ?>></div>
</div>
<?php
$x++;
}
...
<div style="margin-top:8px;"><input type="submit" name="postsal" value="post"></div>
<br>
Pending
<div class="LiHeader">
<div>
...
<?php
$select = "select * from cronjob where type = 'sal' and `status` = 2";
$result = mysql_query($select, connect());
$x = 0;
while ($row = mysql_fetch_array($result,MYSQL_ASSOC)){
$style = "";
...
<div>
	...
	<div id="h3"><?php echo $row['name']; ?></div>
	<div id="h4"><?php echo $row['amount']; ?></div>
	<div id="h5"><input type="submit" name="delsal" value="Delete" onclick="return ondeletez('<?php echo $row['id']; ?>');"></div>
</div>
<?php
$x++;
}
...
<div id="help" class="help">Create a Excel file with the exact header as follows Employee ID, New Salary, Date. Then save it as XML file</div>
