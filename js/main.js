...
function openwindowpay(url,h,w){
	var pay_id = document.getElementById("id");
	pi = "";
	for (var i = 0; i < pay_id.options.length; i++){
		if (pay_id.options[i].selected){
			paycode = pay_id.options[i].value;
			pi = pi + paycode + "@@";
			}
		}

	var id = document.getElementById("id");
	window.open(url+"&idd="+id.value+"&iddx="+pi,'',"toolbar=no,alwaysRaised=yes,location=no,directories=no,status=no,menubar=no,scrollbars=yes,height="+h+",width="+w+",modal=1");
	}

function openwindow_timeattendance(url,h,w){
	var pay_id = document.getElementById("id");
	pi = "";
	for (var i = 0; i < pay_id.options.length; i++){
		if (pay_id.options[i].selected){
	...
	window.open(url+"&idd="+pi,'',"toolbar=no,alwaysRaised=yes,location=no,directories=no,status=no,menubar=no,scrollbars=yes,height="+h+",width="+w+",modal=1");
	}

function openwindowx(url,h,w){
	window.open(url,'',"toolbar=no,alwaysRaised=yes,location=no,directories=no,status=no,menubar=no,scrollbars=yes,height="+h+",width="+w+",modal=1");
	}
...
function createpayroll(url,h,w){
	...
	url = url + "&date1="+f.value+"&date2="+t.value+"&cid="+cid.value+"&payday=" + payday +"&pay_id=" + pi + "&type=" + type.value + "&days=" + days.value+"";
...
function GenFrame(url,id){
 ...
 function picupload(upload_field){
	var re_text = /\.gif|\.txt|\.png|\.pdf|\.doc|\.xls|\.jpg/i;
...
function getHtmlCopyDay(parent){
	x = findPosX(parent) -385;
	y = findPosY(parent) +0;
	var copyto = document.getElementById('copydayx');
	var date1 = document.getElementById('date1x');
	var date2 = document.getElementById('date2x');
...
function onclknet(type, thIs, emid, xxx){
	...
	var textnet = document.getElementById('textnet'+emid);
	var texttin = document.getElementById('texttin'+emid);
	var textearnings = document.getElementById('textearnings'+emid);
	...
	earnings.innerHTML = textearnings.value;
	...
	grosstaxable =  grosstaxable - parseFloat(textsss.value) - parseFloat(textph.value) - parseFloat(textpi.value)
	grossaftertax = grosstaxable - texttin.value;
	tin = parseFloat(texttin.value);

	grosstaxable = grosstaxable ? grosstaxable : 0;


	grossaftertax = grossaftertax ? grossaftertax : 0;


	// if(parseFloat(grosstaxable) <=0 ){
	// 	grosstaxable = 0;
	// }
	// if(parseFloat(grossaftertax) <=0 ){
	// 	grossaftertax = 0;
	// }

	//console.log(grosstaxable))
...
function OpenGetID(parent,type,posX,posY){
	var id = document.getElementById('getid');
	var extra = document.getElementById('extra');
	var ty = document.getElementById('type');
	var x = xstooltip_findPosX(parent) + posY;
	var y = xstooltip_findPosY(parent) + posX;
	...
	if(id.style.visibility == 'visible'){
		...
		if(type.value=='phq'){
			extra.innerHTML="\
				...
				<tr> \
				<td>OR</td> \
				<td><input type='text' id='or' style='width:100px;'></td> \
				</tr>\
				<tr> \
				<td>Transaction Date</td> \
				<td><input type='text' id='td' style='width:100px;'></td> \
				</tr>\
				<tr> \
				<td>Applicable Period</td> \
				<td><input type='text' id='ap' style='width:150px;' maxlength='9'> Max 9 Characters</td> \
				</tr>\
				</table>";
			}
		if(type.value=='sssm'){
			extra.innerHTML="\
				...
				<tr> \
				<td>Date</td> \
				<td><input type='text' id='dt' style='width:100px;' value='" + d +  "'></td> \
				</tr>\
				<tr> \
				<td>Date Paid</td> \
				<td><input type='text' id='dp' style='width:100px;' value='" + d +  "'></td> \
				</tr>\
				</table>";
			}
		if(type.value=='sssl'){
...
function GenReport(){
	...
	y = 0;
	for(x=0;x<parseInt(count.value);x++){
		var ch = document.getElementById('ch'+x);
		var compa = document.getElementById('compa'+x);
		if(ch.checked == true){
			id = id+ch.value + "@@";
			if(y==0){
				cmpid = compa.value;
				}
			if(cmpid!=compa.value){
				cmp=1;
				}
			y++;
			}
	...
	if(type.value=='phq'){
		...
		if(cmp==1 || id == ""){
			alert('choose only 1 company');
			}
		else if (td.value==""){
			alert('Transaction Date is Empty');
			}
		else if (ap.value==""){
			alert('Applicable Period is Empty');
			}
		else if (or.value==""){
			alert('OR is Empty');
			}
		else{
			openwindowx('python/ph_remit.php?vars='+id+'&compa='+cmpid+'&td='+td.value+'&ap='+ap.value+'&or='+or.value,screen.height,screen.width)
			}
		}
	...
	if(type.value=='piq'){
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/pi_remit.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
			}
		}
	...
	if(type.value=='pis'){
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/pi_loan.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
			}
		}
	...
	if(type.value=='sssm'){
		...
		if(cmp==1 || id == ""){
			...
			alert('Date Paid is Empty');
			}
		else if (or.value==""){
			alert('OR is Empty');
			}
		else{
			openwindowx('reports/sss_contrib.php?vars='+id+'&compa='+cmpid+'&or='+or.value+'&dt='+dt.value+'&dp='+dp.value,screen.height,screen.width)
			openwindowx('reports/sss_contri.php?vars='+id+'&compa='+cmpid+'&or='+or.value+'&dt='+dt.value+'&dp='+dp.value,screen.height,screen.width)
			}
		}
	...
	if(type.value=='sssl'){
		...
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/sss_loan.php?vars='+id+'&compa='+cmpid+'&sbr='+or.value+'&date='+dp.value,screen.height,screen.width);
			openwindowx('reports/sss_loan_gofile.php?vars='+id+'&compa='+cmpid+'&sbr='+or.value+'&date='+dp.value,screen.height,screen.width);
			openwindowx('reports/sss_loan_xls.php?vars='+id+'&compa='+cmpid+'&sbr='+or.value+'&date='+dp.value,screen.height,screen.width);
			}
		}
	...
	if(type.value=='acctn'){
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/acctn.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
			}
		}
	if(type.value=='acct'){
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/acct.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
			}
		}
	...
	if(type.value=='dnt'){
		if(cmp==1 || id == ""){
			...
			openwindowx('reports/dnt.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
			}
		}

	if(type.value=='spp'){
		openwindowx('reports/spp.php?vars='+id+'&compa='+cmpid,screen.height,screen.width)
		}
	}

function OpenUpload(parent,posX,posY){
	var id = document.getElementById('getupload');
...
function OpenCalz(parent,type,posX,posY){
	var id = document.getElementById('calz');
	var x = xstooltip_findPosX(parent) + posY;
	var y = xstooltip_findPosY(parent) + posX;
	var ty = document.getElementById('type');
	var genall = document.getElementById('genall');

	ty.value=type;

	if(type=='sssn'){
		genall.style.display='none';
		}
	else{
...
function GenReportCalz(){
	var f = document.getElementById('fdate');
	var t = document.getElementById('tdate');
	var type = document.getElementById('type');
	var compa = document.getElementById('compa');
	var emid = document.getElementById('em_id_calz');
	if(type.value=='sssn'){
		if(f.value=="" || t.value == ""){
			alert('Select a date range');
			}
		else{
			openwindowx('python/sss_new_reg.php?fdate='+f.value+'&tdate='+t.value+'&compa='+compa.value,screen.height,screen.width)
			}
		}
	if(type.value=='pep'){
		if(f.value=="" || t.value == ""){
			...
			openwindowx('reports/pep.php?fdate='+f.value+'&tdate='+t.value+'&compa='+compa.value+'&emid='+emid.value,screen.height,screen.width)
			}
		}
	}

function GenReportCalzAll(){
	var f = document.getElementById('fdate');
	var t = document.getElementById('tdate');
	var type = document.getElementById('type');
	var compa = document.getElementById('compa');
	...
	if(type.value=='pep'){
		if(f.value=="" || t.value == ""){
			...
			openwindowx('reports/pepall.php?fdate='+f.value+'&tdate='+t.value+'&compa='+compa.value+'&emid='+emid.value,screen.height,screen.width)
			}
		}
	}
...
function onLoad(){
	...
	ld.style.width=screen.width;
	ld.style.height=screen.height;
	}

function OnCenter(width,height){
	sz = getWH();
	wd = sz[0];
	ht = sz[1];
	y=(parseInt(wd) / 2) - (parseInt(width) / 2);
	x=(parseInt(ht) / 2) - (parseInt(height) / 2);
	return [x,y];
	}

function CoverIt(){
	var div = document.getElementById('coverit');
	var htmlheight = document.body.parentNode.scrollHeight;
	var htmlwidth = document.body.parentNode.scrollWidth;
	div.style.visibility='visible';
	div.style.width=htmlwidth+"px";
	div.style.height=htmlheight+"px";
	}
...
function DeleteIt(idx){
	...
	ht = 140;
	CoverIt()
	var div = document.getElementById('deleteit');
	var con = document.getElementById('delcontainer');
	...
	con.innerHTML="\
		...
		<br>\
		<div>Deleting Post Number : <b>"+idx+"</b></div>\
		<br>\
		<table> \
		<tr> \
		<td>User Name</td> \
		<td><input type='text' id='uname' name='uname' style='width:100px;'></td> \
		</tr>\
		<tr> \
		<td>Password</td> \
		<td><input type='text' id='passwd' name='passwd' style='width:100px;'></td> \
		</tr>\
		<tr> \
		<td colspan='2'> \
		<br>\
		<input type='hidden' name='del_id' value='"+idx+"'>\
...
function timekeppingexl(){
	var keyword = document.getElementById('keyword').value;
	var filter = "";
	var from = document.getElementById('fyy').value + "-" + document.getElementById('fmm').value + "-" + document.getElementById('fdd').value;
	var to = document.getElementById('tyy').value + "-" + document.getElementById('tmm').value + "-" + document.getElementById('tdd').value;
	self.location = "reports/timeattendance-exl.php?from="+from+"&to="+to+"&keyword="+keyword+"&filter="+filter;
	}
...
function onBPIpdf(){
	...
	openwindowx('../python/bpipdf.php?id='+id+'&tdz='+new_day,screen.height,screen.width)
	}

function getSalary(emid){
	requestObject = xmlHttpRequestHandler.createXmlHttpRequest();
	requestObject.onreadystatechange=onReadyStateChangeResponseGetSalary;
	requestObject.open("Get",'getsalary.php?emid='+emid, true);
	requestObject.send(null);
	}

function onReadyStateChangeResponseGetSalary(){
	var ready, status;
	try{
		ready = requestObject.readyState;
		status = requestObject.status;
	...
	if(ready == 4 && status == 200){
		...
		hm = mins.value.split(":");
		if(name.value=='NIGHT DIFF'){
			hamt = (parseFloat(txt) * parseFloat(hm[0])) * .1;
			mamt = ((parseFloat(txt)/60) * parseFloat(hm[1])) * .1;
			}
		else{
			hamt = parseFloat(txt) * parseFloat(hm[0]);
			mamt = (parseFloat(txt)/60) * parseFloat(hm[1]);
			}

		if(parseFloat(hamt+mamt) > 0){
			amt.value = (hamt+mamt).toFixed(2);
			}
		else{
			amt.value = 0;
			}
...
function getSalaryHP(emid){
	requestObject = xmlHttpRequestHandler.createXmlHttpRequest();
	requestObject.onreadystatechange=onReadyStateChangeResponseGetSalaryHP;
	requestObject.open("Get",'gethp.php?emid='+emid, true);
	requestObject.send(null);
	}

function onReadyStateChangeResponseGetSalaryHP(){
	var ready, status;
	try{
		ready = requestObject.readyState;
		status = requestObject.status;
	...
	if(ready == 4 && status == 200){
		...
		var amt = document.getElementById('amount');
		var days = document.getElementById('days');
		hp=parseFloat(days.value) * parseFloat(txt);
		if(parseFloat(hp) > 0){
			amt.value = hp.toFixed(2);
			}
		else{
			amt.value = 0;
			}
...
function payrollRegister(){
	...
	var smm = document.getElementById("smm");
	var payday = svar.value + syy.value + "-" + smm.value;
	self.location='reports/register_xls.php?payday='+ payday;
}

function payrollRegisterDetails(id=0){
	var svar = document.getElementById("svarjv");
	var syy = document.getElementById("syyjv");
	var smm = document.getElementById("smmjv");
	var payday = svar.value + syy.value + "-" + smm.value;
	var compa = document.getElementById("scompa").value;
	var type = document.getElementById("type");
	var type = type.value;
	self.location='reports/pay_register_details.php?payday='+ payday+'&compa='+compa+'&type='+type+'&id='+id;
}
...
function jv(){
	...
	var type = document.getElementById("type");
	var type = type.value;
	if(parseInt(smm.value) <= 6 && syy.value == '2017' && svar.value !=  ''){
		alert('May 2017 Downwards must use Whole month.');
	} else {
		if(type=='HALF 13TH MONTH'){
			self.location='reports/jv_13_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
		} else {
			if(compa+0 == 7){
				self.location='reports/jv_xls_ibmi.php?payday='+ payday+'&compa='+compa+'&type='+type;
			} else {
				self.location='reports/jv_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
			}
		}
	}
}
...
function jvqne(){
	...
	if(parseInt(smm.value) <= 6 && syy.value == '2017' && svar.value !=  ''){
		...
		if(type=='HALF 13TH MONTH'){
			self.location='reports/jvqne_13_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
		} else {
			if(compa+0 == 7){
				self.location='reports/jvqne_xls_ibmi.php?payday='+ payday+'&compa='+compa+'&type='+type;
			} else {
				self.location='reports/jvqne_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
			}
		}
	}
}
...
function sss_contri(){
	...
	var type = document.getElementById("type");
	var type = type.value;
	self.location='reports/sss_contri_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
}
...
function deductions(){
	...
	var type = document.getElementById("type");
	var type = type.value;
	self.location='reports/deduction_xls.php?payday='+ payday+'&compa='+compa+'&type='+type;
}
...
function forecast(){
	...
	if(parseInt(smm.value) <= 6 && syy.value == '2017' && svar.value !=  ''){
		...
		self.location='reports/employee_forecast.php?payday='+ payday+'&compa='+compa+'&type='+type;
	}
}

function alphalist(){
	var compa = document.getElementById("scompa1").value;
	var type = document.getElementById("type1").value;
	var year = document.getElementById("alpha_year").value;
	self.location='reports/alphalist.php?year='+year+'&compa='+compa+'&type='+type+'&year='+year;
}


function alphalist_total(){
	var compa = document.getElementById("scompa1").value;
	var type = document.getElementById("type1").value;
	var year = document.getElementById("alpha_year").value;
	self.location='reports/alphalist_total.php?year='+year+'&compa='+compa+'&type='+type+'&year='+year;
}
...
function bonusComparison(){
	...
	var type = document.getElementById("type1").value;
	var year = document.getElementById("alpha_year").value;
	self.location='reports/bonus.php?year=2017&compa='+compa+'&type='+type+'&year='+year;
}

function m13Comparison(){
	var compa = document.getElementById("scompa1").value;
	var type = document.getElementById("type1").value;
	var year = document.getElementById("alpha_year").value;
	self.location='reports/m13.php?year=2017&compa='+compa+'&type='+type+'&year='+year;
}
...
function otReport(){
	...
	if(parseInt(smm.value) <= 6 && syy.value == '2017' && svar.value !=  ''){
		...
		self.location='reports/employee_ot_report.php?payday='+ payday+'&compa='+compa+'&type='+type;
	}
}
