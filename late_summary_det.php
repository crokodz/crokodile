...
<table width=100% border="0" cellpadding="4" cellspacing="0" rules="all">
	<tr>
		...
		<td width=300px align="center">Remarks</td>
		<td align="center">&nbsp;</td>
	</tr>
	
	<?php
	$select = "select trxn_date, sum(late) as lt, sum(ut) as ut from transaction left join employee using (`em_id`) where trxn_date between '" . $_GET['fdate'] . "' and '" . $_GET['tdate'] . "' and (late > 0 or ut > 0) and employee.status = 'active' and transaction.em_id = '" . $_GET['em_id'] . "' group by trxn_date order by trxn_date desc" ;
	$result = mysql_query($select, connect());
