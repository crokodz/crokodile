...
function getadjustments($em_id){
	$select = "select * from employee_taxable where 
		em_id = '" . $em_id . "' and 
		status != 'deleted' and (name='LHRD' or  name= 'ADDITIONAL DAY')
		order by id";
...
<h3 class="wintitle">Employee Holiday Pay</h3>
<form method="post">
...
<tr>
	<td width=20%>
		...
		function getSelectedId(text, li) {
			...
			self.location='index.php?menu=holidaypay&idd='+myData[0];
			}
		</script>
	</td>
</tr>
...
<tr>
	<td width=80%>
		...
		<?php
		$select = "select * from taxable_entry where name='LHRD'  or name= 'ADDITIONAL DAY' order by name asc";
		...
		|
		Days <input type="text" name="days" id="days" style="width:70px;"  onkeyup="getSalaryHP('<?php echo $_GET['id'];?>');">
	</td>
	<td width=20%>Amount <input type="text" name="amount" id="amount" style="width:100px;"></td>
</tr>
<tr>
...
if (isset($_POST['search'])){
	...
	echo '<script>';
	if (empty( $_GET['page'])){
		echo 'self.location = "index.php?menu=holidaypay&page=1&keyword=' . $keyword . '&company=' . $company . '";';
		}
	else{
		echo 'self.location = "index.php?menu=holidaypay&page=' . $_GET['page'] . '&keyword=' . $keyword . '&company=' . $company . '";';
		}
	echo '</script>';
	}
...
$pagelist = $p->pageList($_GET['page'], $pages,'index.php?menu=holidaypay&keyword=' . $_GET['keyword'] . '&company=' . $_GET['company']);
##
...
?>
	...
	<tr style="cursor:pointer;" id="emlist">
		<td align="left" onclick="self.location='index.php?menu=holidaypay&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['em_id']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=holidaypay&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['name']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=holidaypay&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['department']; ?></td>
		<td align="left" onclick="self.location='index.php?menu=holidaypay&idd=<?php echo $row['em_id']; ?>';"><?php echo $row['em_number']; ?></td>
	</tr>
	<?php
	$x++;
	}
