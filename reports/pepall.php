...
$select = " select tb3.`name`, tb2.`division`, tb2.`pay_id`,tb2.`lname`,tb2.`fname`, tb2.`mname`, tb1.`salary`, tb2.`salary_based`, tb1.`absent`, tb1.`late`,
	...
	from `posted_summary` tb1 left join employee tb2 using(`em_id`) left join company tb3 on(tb1.`company_id` = tb3.`id`) where tb1.company_id = '" . $_GET['compa'] . "' and tb1.`from` >= '" . $_GET['fdate'] . "' and tb1.`to` <= '" . $_GET['tdate'] . "' order by tb1.`to` asc";
$result = mysql_query($select, connect());
