...
$ymd = date("FjYgia");
$excel=new ExcelWriter("jv_13_" . $ymd . ".xls");
...
if($company_id == 6 || $company_id == 1){
	...
	sum(t1.nontax) as nontax,
	group_concat(DISTINCT(t1.posted_id) SEPARATOR ',') as posted_id
	from posted_summary t1 left join employee t2 on (t1.em_id = t2.em_id) where t1.post_type='" . $_GET['type'] . "' AND t1.payday like '%" . $_GET['payday'] . "' and t1.company_id = " . $company_id . "  group by t2.department";
}
...
while($row = mysql_fetch_array($result)){
  ...
  $select1 = "select t2.pay_id, t2.tin, t2.fname, t2.mname, t2.lname, t1.em_id, sum(t1.sss) as sss, sum(t1.taxable_salary) as taxable_salary, sum(tax) as tax, sum(t1.salary) as salary, sum(t1.nontax) as nontax,
...
if($company_id == 6 || $company_id == 1){
   	$select1 = "select t2.salary as em_salary, t2.tin, t2.pay_id, t2.fname, t2.mname, t2.lname, t1.em_id, sum(t1.sss) as sss, sum(t1.taxable_salary) as taxable_salary, sum(tax) as tax, sum(t1.salary) as salary, sum(t1.nontax) as nontax,
	group_concat(posted_id) as posted_id, sum(t1.ph) as ph, sum(t1.salary) as salary, t2.pdm, sum(t1.pi) as pi
	from posted_summary t1 left join employee t2 on (t1.em_id = t2.em_id) where t1.post_type='" . $_GET['type'] . "' AND  t1.payday like '%" . $_GET['payday'] . "' and t1.company_id = " . $company_id . " and t2.department = '" . $row['department']  . "'  group by t1.em_id";
}
	$result1 = mysql_query($select1, connect());
	...
	$select2x = "select  sum(t1.amount) as amount, t1.name from employee_taxable t1 left join employee t2 on (t1.em_id = t2.em_id) where t2.department = '" . $row['department']  . "' and  t1.posted_id in (" . $row['posted_id'] . ") group by t1.name";
	$result2x = mysql_query($select2x, connect());
	while($row2x = mysql_fetch_array($result2x)){
		$excel->writeLine(array('', $row2x['name'] , '',roundoff($row2x['amount']),''));
		$tax2x = $row2x['amount'];
	...
	$sw = ($row['taxable_salary'] - $taxtx) - ($row['ot']+$dwopt);
	$excel->writeLine(array($idc.'-3101', 'SALARIES AND WAGES' , '','0.00',''));
	...
	//if($total > $total_row){
		$excel->writeLine(array($idc.'', '13Th Month' , '',roundoff(($total - $total_row)  + $sw ) ));
	//}
...
header('Content-Disposition: attachment; filename='.'jv_13_' . $ymd . '.xls');
header("Location: " ."jv_13_" . $ymd . ".xls");
exit;
